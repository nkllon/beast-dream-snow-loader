...
def get_class_hierarchy(client: ServiceNowAPIClient, table_name: str, depth: int = 0) -> list[str]:
    ...
    super_class_id = super_class_link.get("value") if isinstance(super_class_link, dict) else None
    
    if super_class_id and depth < 5:
        super_class_info = client.query_records("sys_db_object", query=f"sys_id={super_class_id}", limit=1)
        if super_class_info:
...
def main():
    ...
    tables = ["cmdb_ci_nat_gateway", "cmdb_ci_internet_gateway", "cmdb_ci_site", "cmdb_ci_network_node"]
